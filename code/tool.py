...
def load_cache(cache_path, line_based=False, is_json=True):
...
def persist_cache(persist_path, cache, line_based=False, is_json=True):
    with open(persist_path, 'w') as of:
        if line_based:
            if not isinstance(cache, (list, tuple)):
                logger.warning('linebased cache only suit for list type, cache not persisted')
            else:
                for item in cache:
                    if is_json:
